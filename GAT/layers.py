...
class GraphAttentionLayer(nn.Module):
    ...
    def __init__(self, in_features, out_features, dropout, alpha, teleport_probability, concat=True):
        ...
        #self.link_prediction_layers = LinkPrediction(graph_embedding_size * num_edge_cat, layers).to(device)
        self.a_link = nn.Parameter(torch.empty(size=(out_features, 1)))


    def _link_prediction(self, h, mini_batch = False):
        if mini_batch == True:pass
        else:

            h = torch.matmul(h.unsqueeze(1) - h.unsqueeze(0), self.a_link)
