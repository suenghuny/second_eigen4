...
class Agent(nn.Module):
    ...
    def learn(self, cum_losses_old, graph_learning_stop):
        ...
        params_to_freeze = list(self.node_representation.parameters()) + \
                           list(self.node_representation_comm.parameters()) + \
                           list(self.func_obs.parameters()) + \
                           list(self.func_glcn.parameters())+ \
                           list(self.action_representation.parameters())

        #print(self.node_representation.fcn_1.weight[0])
        ...
        if cfg.given_edge == True:
            ...
            loss = rl_loss+graph_loss
        if self.eps_clip >= 100:
            self.eps_clip *= 0.99
        # 첫 번째 그룹의 파라미터들을 리스트로 모읍니다

        loss.backward()
        # for param in params_to_freeze:
        #     param.requires_grad = False
        # print(self.action_representation.fcn_1.weight[1][0])
        # print(self.func_obs.a[1][0])
        # print(self.func_glcn.a[1][0])
        # print(self.func_glcn.a_link[1][0])
        # print(self.Q.fcn_3.weight[1][0])
        # print("================")
        ...
        if self.optimizer.param_groups[0]['lr'] >= cfg.lr_min:
            self.scheduler.step()
        self.original_loss = loss.detach()
