...
if sys.platform == "linux":
    ...
    os.environ.setdefault("SC2PATH", "/root/second_eigen4/3rdparty/StarCraftII")
...
def evaluation(env, agent, num_eval):
    ...
    for e in range(num_eval):
        ...
        while (not done) and (step < max_episode_len):
            ...
            agent_feature = torch.tensor(node_feature)[:num_agent, :-1]#torch.concat([torch.tensor(node_feature)[:num_agent, :-1], action_history.to('cpu')], dim=1)
    ...
    for e in range(num_episode):
        ...
        if e % 10 == 1:
            if vessl_on == True:
                ...
                writer.add_scalar("reward", np.mean(epi_r), e)
                writer.add_scalar("lap_quad", np.mean(lap_quad), e)
                writer.add_scalar("sec_eig",  np.mean(sec_eig), e)
                writer.add_scalar("rl_lo", np.mean(rl_lo), e)
                writer.add_scalar("q_t", np.mean(q_t), e)
                lap_quad = []
                sec_eig = []
                epi_r = []
                rl_lo = []
        ...
        if eval == True:
            ...
            if vessl_on == True:
                ...
                writer.add_scalar("win_rate", win_rate, t)
                wr_df = pd.DataFrame(win_rates)
